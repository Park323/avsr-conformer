...
def build_model(config, vocab):
    ...
    if model_name=='las':
        ...
        model = build_conf_model(config, vocab=vocab, mode='multi')
    elif model_name=='conf_a':
        model = build_conf_model(config, vocab=vocab, mode='audio')
...
def build_conf_model(config, vocab, mode, *args, **kargs):
    if mode=='multi':
